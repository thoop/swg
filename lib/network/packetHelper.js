...
packetHelper.parseObjIntoPacket = function(packetObj) {
	if(!packetObj || !packetObj.name) {
		console.log('Tried to send packet without a "name". Ignoring...', packetObj);
		return {};
	}

	var newPacket = {};

	if(this.packets[packetObj.name.toLowerCase()]) {

		return this.packets[packetObj.name.toLowerCase()].parseObjIntoPacket(packetObj);

	} else {
		console.log('WARNING: no packet found for', packetObj.name);
	}
