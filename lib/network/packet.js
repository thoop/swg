...
packet._checkAndRemoveCRC = function(options, callback) {
	...
	var newBuffer = Buffer.allocUnsafe(options.buffer.length - options.crcLength);
...
packet._encrypt = function(options, callback) {
	var bufferWithoutOpcode = Buffer.allocUnsafe(options.buffer.length - 2);
	...
	var newBuffer = Buffer.allocUnsafe(options.buffer.length);
...
packet._inflateAndRemoveCompressionFlag = function(options, callback) {
	var _this = this;
	var bufferWithoutOpcodeOrCompressionFlag = Buffer.allocUnsafe(options.buffer.length - 3);
