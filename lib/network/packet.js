		...
		this.operands.forEach(function(value) {
			...
			if (value.type == 'byte') {
				...
				if(value.endian == 'little') {
					parsedPacket[value.name] = options.buffer.readUInt16LE(index);
				} else {
				...
				if(value.endian == 'little') {
					parsedPacket[value.name] = options.buffer.readUInt32LE(index);
				} else {
				...
				index += dataLength + 4;
			} else if (value.type == 'u_string') {
				dataLength = options.buffer.readUInt16LE(index);
				parsedPacket[value.name] = '';
				for(var i = 0; i < dataLength; i++) {
					parsedPacket[value.name] += String.fromCharCode(options.buffer.readUInt16LE(index + 2 + (i*2)));
				}
				index += dataLength + 4;
			}

			if (value.child && value.child.type == 'array') {
				var arrayLength = parsedPacket[value.name];
				if(arrayLength > 0) {
					//loop through and make sure to add to arrayLength to increment index.
					parsedPacket[value.name] = [];
					value.child.operands.forEach(function(operand) {
						//so this is going to have to do a nested version of the above type parsing.
						//so the above code will need to be refactored to allow nested calls to populate children
						// parsedPacket[value.name].push()
					});
					index += dataLength;
				}
			}
	...
	if (this.operands) {
		this.operands.forEach(function(value) {
			...
			if (value.type == 'byte') {
				...
				if(value.endian == 'little') {
					parsedHexStr += Buffer.from(_this._getHexInt(packetObj[value.name], 4), 'hex').swap16().toString('hex');
				} else {
					parsedHexStr += _this._getHexInt(packetObj[value.name], 4);
				}
			} else if (value.type == 'int') {
				if(value.endian == 'little') {
					parsedHexStr += Buffer.from(_this._getHexInt(packetObj[value.name], 8), 'hex').swap32().toString('hex');
				} else {
...
packet._appendCRC = function(options, callback) {
	// return Buffer.from(buffer.toString('hex') + crc(buffer), 'hex');
...
packet._checkAndRemoveCRC = function(options, callback) {
	...
	var newBuffer = Buffer.alloc(options.buffer.length - options.crcLength);
...
packet._encrypt = function(options, callback) {
	var bufferWithoutOpcode = Buffer.alloc(options.buffer.length - 2);
	...
	var newBuffer = Buffer.alloc(options.buffer.length);
...
packet._deflateAndAddCompressionFlag = function(options, callback) {
	if (defaults.useCompression && options.buffer.length > defaults.receiveBufferSize - 20) {
...
packet._inflateAndRemoveCompressionFlag = function(options, callback) {
	var _this = this;
	var bufferWithoutOpcodeOrCompressionFlag = Buffer.alloc(options.buffer.length - 3);
