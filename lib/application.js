...
app.init = function(options) {
    ...
    this.server.on('message', function(buffer, requestInfo) {
        ...
        if (app.options.verbose) {
            console.log('Recieved ' + hex + ' from ' + requestInfo.address + ':' + requestInfo.port);
        }

        var packet = app._parsePacket(hex);
        ...
        var res = {
            ...
            send: function(response) {
                if (response.substring(0,4) != '0002') {
                    response = app._encrypt(response, 'AB8E92D4');
                    response += app._getPacketCRCBytes(response);
                }
...
app._parsePacket = function(hex) {
    var soeOpcode = hex.substring(0, 4);
    var crcSeed = 'AB8E92D4';
...
app._decrypt = function(hex, crcSeed) {
...
app._encrypt = function(hex, crcSeed) {
...
app._getPacketCRCBytes = function(data) {
    var crc = app._crc32(data);
    console.log(crc);
    return '0000';
};

app._crc32 = function(str) {
